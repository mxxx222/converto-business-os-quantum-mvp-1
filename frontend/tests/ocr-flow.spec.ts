...
test.describe('OCR Processing Flow', () => {
  test('should show process button for queued receipts', async ({ page }: { page: import('@playwright/test').Page }) => {
    await page.goto('/receipts');
    // This would need actual queued receipts to test
    await expect(page.getByText('Prosessoi')).toBeVisible();
  });

  test('should display parsed receipt data', async ({ page }: { page: import('@playwright/test').Page }) => {
    await page.goto('/receipts');
