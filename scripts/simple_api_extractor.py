...
def extract_from_browser():
    ...
    try:
        ...
        result = subprocess.run(
            ["osascript", "-e", url_script], capture_output=True, text=True, timeout=5
        )
        ...
        if result.returncode == 0:
            ...
            if "kilocode" in current_url.lower():
                ...
                source_result = subprocess.run(
                    ["osascript", "-e", page_source_script],
                    capture_output=True,
                    text=True,
                    timeout=10,
                )
    ...
    try:
        ...
        if os.path.exists(expanded_ff_dir):
            ...
            if os.path.exists(cookies_file):
                ...
                try:
                    result = subprocess.run(
                        ["strings", cookies_file], capture_output=True, text=True, timeout=10
                    )
                    if result.returncode == 0:
