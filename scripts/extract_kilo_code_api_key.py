...
def main():
    ...
    if not browser_files:
        ...
        print(
            "5. Run this script again with the key: python3 scripts/extract_kilo_code_api_key.py YOUR_API_KEY"
        )
        return
    ...
    if found_keys:
        ...
        if update_configuration(api_key):
            print("🎉 Successfully configured Kilo Code API key!")
        ...
        if len(sys.argv) > 1:
            api_key = sys.argv[1]
            if update_configuration(api_key):
                print("🎉 Successfully configured Kilo Code API key from argument!")
                print(f"   Key: {api_key[:20]}...")
            else:
                print("❌ Failed to update configuration")
        else:
