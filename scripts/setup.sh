...
ADMIN_BASIC_PASS=supersecret
ENV
  echo ">> Wrote frontend/.env.local"
fi

# --- DB INIT (optional, safe no-op if models not present) ---
python - <<'PY' || true
try:
    from shared_core.utils.db import Base, engine
    Base.metadata.create_all(engine)
    print("DB ready (shared_core).")
...
if [ ! -f "package.json" ]; then
  echo ">> Next.js app not found; creating minimal app"
  npm init -y >/dev/null 2>&1
  npm install next@14 react react-dom -s >/dev/null 2>&1
...
export default function Page(){ return <main style={{padding:24}}>Converto™ Dashboard — MVP</main>; }
TSX
fi
# ImpactCard placeholder if missing
if [ ! -f "modules/impact/Card.tsx" ]; then
  mkdir -p modules/impact
...
TSX
  # inject card to homepage if trivial page exists
  if grep -q "Converto™ Dashboard" app/page.tsx 2>/dev/null; then
    cat > app/page.tsx <<'TSX'
...
# --- STRIPE CLI (optional) ---
if command -v stripe >/dev/null 2>&1 && [ -n "${STRIPE_WEBHOOK_SECRET:-}" ]; then
  echo ">> Starting Stripe listen (optional)"
  pkill -f "stripe listen" >/dev/null 2>&1 || true
  nohup stripe listen --forward-to ${BACKEND_HOST}:${BACKEND_PORT}/api/v1/stripe/webhook >/tmp/stripe_listen.log 2>&1 &
else
  echo ">> Stripe listen skipped (stripe CLI not found or secrets not set)"
fi
