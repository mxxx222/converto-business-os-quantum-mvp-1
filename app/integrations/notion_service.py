...
def _create_page(db_id: str, properties: Dict, children: Optional[list] = None) -> Dict:
    ...
    response = requests.post(f"{API_BASE}/pages", headers=HEADERS, json=payload, timeout=10)
...
def push_receipt(data: Dict) -> Dict:
    ...
    properties = {
        ...
        "Vendor": {"title": [{"text": {"content": data["vendor"][:200]}}]},
        "Amount": {"number": float(data["amount"])},
        "Category": {"select": {"name": data.get("category", "Muu")}},
        "Status": {"select": {"name": data.get("status", "New")}},
    }
    ...
    if data.get("image_url"):
        properties["Image"] = {
            "files": [
                {"type": "external", "name": "receipt", "external": {"url": data["image_url"]}}
            ]
        }
...
def upsert_calendar_event(event: Dict) -> Dict:
    ...
    # Build Notion properties
    properties = {
        "Title": {"title": [{"text": {"content": event["title"][:200]}}]},
        "Start": {"date": {"start": event["start"], "end": event.get("end")}},
        "Status": {"select": {"name": event.get("status", "Planned")}},
    }
...
def inventory_create(item: Dict) -> Dict:
    ...
    """
    properties = {
        "SKU": {"title": [{"text": {"content": item["sku"][:200]}}]},
        "Nimi": {"rich_text": [{"text": {"content": item.get("nimi", "")}}]},
        "Kategoria": {"select": {"name": item.get("kategoria", "Muu")}},
        "Varastosaldo": {"number": float(item.get("varastosaldo", 0))},
        "Yksikkökustannus": {"number": float(item.get("yksikkokustannus", 0))},
        "Hälytysraja": {"number": float(item.get("halytysraja", 0))},
        "Toimittaja": {"rich_text": [{"text": {"content": item.get("toimittaja", "")}}]},
    }
...
def inventory_adjust(sku: str, delta: int, notes: str = "") -> Dict:
    ...
    """
    properties = {
        "SKU": {"title": [{"text": {"content": f"{sku} – muutos {delta:+d}"}}]},
        "Nimi": {"rich_text": [{"text": {"content": notes or "Varastomuutos"}}]},
        "Varastosaldo": {"number": float(delta)},
    }
...
def shipment_create(shipment: Dict) -> Dict:
    ...
    """
    properties = {
        "Shipment ID": {"title": [{"text": {"content": shipment["id"][:200]}}]},
        "Status": {"select": {"name": shipment.get("status", "Draft")}},
        "HS-koodi": {"rich_text": [{"text": {"content": shipment.get("hs", "")}}]},
        "Arvo (EUR)": {"number": float(shipment.get("arvo_eur", 0))},
        "ALV %": {"number": float(shipment.get("alv", 0))},
        "Tulli %": {"number": float(shipment.get("tulli", 0))},
        "Lähtömaa": {"rich_text": [{"text": {"content": shipment.get("lahtomaa", "")}}]},
    }

    # Add arrival date if provided
    if shipment.get("saapumapaiva"):
        properties["Saapumispäivä"] = {"date": {"start": shipment["saapumapaiva"]}}

    # Add attachment if provided
    if shipment.get("liite_url"):
        properties["Liitteet"] = {
            "files": [
                {"type": "external", "name": "document", "external": {"url": shipment["liite_url"]}}
            ]
        }
...
def upsert_calendar_event(
    ...
    if page_id:
        ...
        response = requests.post(
            f"{API_BASE}/pages",
            headers=HEADERS,
            json={"parent": {"database_id": calendar_db}, "properties": properties},
            timeout=10,
        )
