...
def _wa_meta(text: str) -> bool:
    ...
    if not all([token, phone_id, to]):
        print(
            "[WhatsApp Meta] Missing configuration (WA_META_TOKEN, WA_META_PHONE_ID, WA_TO_PHONE)"
        )
        return False
...
def _wa_twilio(text: str) -> bool:
    ...
    if not all([sid, token, from_number, to]):
        print(
            "[WhatsApp Twilio] Missing configuration (TWILIO_SID, TWILIO_TOKEN, WA_TWILIO_FROM, WA_TO_PHONE)"
        )
        return False
...
def _slack_send(text: str, tenant_id: str) -> bool:
    ...
    data = {"text": text, "username": "Converto Reminders", "icon_emoji": ":bell:"}
